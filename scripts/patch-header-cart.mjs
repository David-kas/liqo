/**
 * Встраивает корзину в HTML шапки на всех страницах.
 * Запуск: node scripts/patch-header-cart.mjs
 */
import fs from 'fs';
...
const CART_BTN = `<button type="button" class="header-cart-btn" id="site-cart-btn" aria-label="Корзина">
...
for (const fp of walk(ROOT)) {
  ...
  next = next.replace(/\s*<button type="button" class="btn-header-pill btn-header-cart"[\s\S]*?<\/button>/g, '');

  if (!next.includes('id="site-cart-btn"')) {
    const idx = next.indexOf(MENU_BTN);
    if (idx !== -1) {
      next = next.slice(0, idx) + '\n    ' + CART_BTN + '\n    ' + next.slice(idx);
    }
  }

  if (next.includes('id="cart-panel"')) {
    next = stripCartBlocks(next);
  }
...
console.log(`patch-header-cart: updated ${changed} html files`);
