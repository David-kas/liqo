...
let changed=0; for (const file of walk(ROOT)) { const before=fs.readFileSync(file,'utf8'), after=normalizePhone(normalizeDomains(before)); if(after!==before){fs.writeFileSync(file,after,'utf8');changed++;} }
fs.writeFileSync(path.join(ROOT,'sitemap.xml'),generateSitemap(),'utf8');
fs.writeFileSync(path.join(ROOT,'robots.txt'),`User-agent: *\nAllow: /\n\nSitemap: ${NEW_ORIGIN}/sitemap.xml\nHost: ${NEW_ORIGIN}\n`,'utf8');
console.log(`LIQO preparation complete: ${changed} files normalized; sitemap generated.`);
